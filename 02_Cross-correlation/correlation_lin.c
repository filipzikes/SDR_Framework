 ...
 * SEQUENCE ARRAYS
********************/
cplxarr_double* sequences;
char* seq_path = NULL;
size_t Sequence_Size = 0;
int seq_num = 0;

void create_arrays(int n) {
    sequences = calloc(n, sizeof(cplxarr_double));
    ptrchk(sequences, "malloc", "cplxarr_double[]");
    for (int i = 0; i < n; i++) {
        create_cplx_arr_double(&(sequences[i]), Sequence_Size);
        if (sequences[i].size != Sequence_Size) {
            printf(" (!) Error allocating %d. cplxarr_double\n", i+1);
            call_shutdown(111);
        }
    }
}
void load_sequence(const char* filename, cplxarr_double arr) {
    FILE * File_Data;
    ...
    while ((getline(&line, &len, File_Data) != -1) && (j < Sequence_Size)) {
        ...
        arr.real[j] = (double) val_i;
        arr.imag[j] = (double) val_q;
        j++;
    }
    free_str(&line);
    fclose(File_Data);
...
int main () {
    ...
    if (seq_num > 1) {
        cplxarr_double corr = {}; create_cplx_arr_double(&corr, 2*Sequence_Size-1);
        if (corr.size != 2*Sequence_Size-1) {
            printf(" (!) Error allocating cplxarr_double\n");
            call_shutdown(10);
        }
        
        timer_start();
        xcorr(&(sequences[0]), &(sequences[1]), &corr);
        timer_stop();
        timer_print_full();
        ...
        fd = fopen("out.txt", "w");
        if (fd != NULL) {
            for (size_t j = 0; j < corr.size; j++) {fprintf(fd, "\t%e\t%e\n", corr.real[j], corr.imag[j]);}
            fclose(fd);
        }
    }
